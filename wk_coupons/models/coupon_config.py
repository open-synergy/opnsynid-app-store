...
class VoucherConfig(models.Model):
	...
	default_validity = fields.Integer(string='Validity(in days)', default=-1, help="Validity of this Voucher in days( -1 for unlimited )")
	default_availability = fields.Integer(string='Total Available', default=-1, help="Total availability of this voucher( -1 for unlimited )")
	default_value = fields.Float(string='Voucher Value', default=0.0)
	...
	def _get_default_values(self):
		...
		if not self_obj:
			raise osv.except_osv(('Error!'), ("Sorry, Please configure the module before creating any voucher."))
		temp_dict = {
	...
	def create(self, vals):
		self_obj = self.search([('active','=',True)])
		if self_obj:
			raise osv.except_osv(('Error!'), ("Sorry, you can not create more than one configuration."))
		if vals.get('min_amount') < 0 or vals.get('max_amount') < 0 :
			raise osv.except_osv(('Error!'), ("Minimum or maximum value should not be negative.'"))

		if  vals.get('max_amount') < vals.get('min_amount'):
			raise osv.except_osv(('Error!'), ("Maximum value should not be greater than minimum value."))

		if vals.get('default_value') < vals.get('min_amount'):
			raise osv.except_osv(('Error!'), ("Voucher value should be greater than minimum value"))

		if vals.get('default_value') > vals.get('max_amount'):
			raise osv.except_osv(('Error!'), ("Voucher value should be less than maximum valu"))

		if vals.get('default_value') <= 0:
			raise osv.except_osv(('Error!'), ("Voucher value should be greater than 0."))

		if  vals.get('min_amount') <= 0:
			raise osv.except_osv(('Error!'), ("Minimum value should not be <= 0"))
	...
	def write(self, vals):
		...
		if vals.has_key('max_amount') and vals['max_amount'] <=0:
			raise osv.except_osv(('Error!'), ("Maximum value should not be <= 0"))
		if vals.get('min_amount') and vals.get('max_amount') and vals.get('min_amount') >  vals.get('max_amount'):
			raise osv.except_osv(('Error!'), ("Maximum value should not be less than minimum value."))
		...
		if vals.get('max_amount') and vals.get('max_amount') < self.min_amount:
			raise osv.except_osv(('Error!'), ("Maximum value should be greater than minimum value."))
		if vals.get('default_value'):
			...
			if vals.get('max_amount'):
				if vals.get('default_value') > vals.get('max_amount'):
					raise osv.except_osv(('Error!'), ("Voucher value should be less than maximum value"))
